...
public class TwsDomTree extends TreeWrapper{
	...
	private ArrayList<MenuMaker> menuMakers = new ArrayList<MenuMaker>();
	private ArrayList<KeyAccelerator> keyAccelerators =  new ArrayList<KeyAccelerator>();
	...
	public TwsDomTree(Composite parent, int style){
		...
		addListener(SWT.SetData, new Listener() {
			public void handleEvent(Event event) {
				...
				if(d==1 && event.index==0){ // create fake 'Attributes' item
					...
					}else node = nodes[event.index-d];
					addNodeInTree(item,node,null);
					if(parent==null) item.setExpanded(true);
				}
			}
		});
		addFocusListener(new FocusListener(){
			...
			public void focusLost(FocusEvent e) {
				if(selectedTreeItem!=null) deselectAll();
			}
		});
	}
	...
	public boolean addNodeInTree(Object parent, Node node, IProgressMonitor monitor){
		...
		case Node.TEXT_NODE  :
			...
			tItem.setText(node.getNodeValue().trim()); break;
		case Node.ATTRIBUTE_NODE:
			tItem.setImage(imageAttrib);
			String str = node.getNodeName()+ "=\"" +node.getNodeValue()+"\"";
			tItem.setText(new String[]{str,str}); break;
		case Node.ENTITY_NODE:
			tItem.setText("[Entity]"); break;
		case Node.ENTITY_REFERENCE_NODE :
			tItem.setText("[Entityref]"); break;
		case Node.PROCESSING_INSTRUCTION_NODE :
			tItem.setText("[Pi]"); break;
		case Node.COMMENT_NODE :
			tItem.setText("[Comment]");	break;
		case Node.DOCUMENT_FRAGMENT_NODE :
			tItem.setText("[Docfgmt]");	break;
		case Node.DOCUMENT_TYPE_NODE :
			tItem.setText("[Doctype]");	break;
		case Node.NOTATION_NODE :
			tItem.setText("[Notation]"); break;
		default: break;
		}
	...
	public void selectTreeItem(TreeItem ti){
		if(selectedTreeItem!=null) selectedTreeItem.setBackground(Display.getDefault().getSystemColor(SWT.COLOR_LIST_BACKGROUND));
		selectedTreeItem = ti;
	...
	private void buildContextMenu(MouseEvent e){
		...
		if(menu.getItemCount()>0)menu.setVisible(true);
		else menu.dispose();
	}
	...
	public Document getDocument(){
		...
		if(top != null){
			...
			if(obj != null) return obj.getOwnerDocument();
		}
		return null;
	}
