...
public class DreamFaceAddActionAction extends DreamFaceAbstractAction {

	public DreamFaceAddActionAction() {
		super();
	}
	...
	public void run() {
        ...
        try {
    		...
    		if (explorerView != null) {
    			...
    			MashupActionDialog mashupDialog = new MashupActionDialog(shell, MashupActionDialog.TYPE_ADD, mdc, mac);
	        	mashupDialog.open();
	    			...
	    			if ((dataview != null) && (!dataview.equals(""))) {
	    				if (securedLogin()) {
		    				try {
				    			...
				    			String dfResponse = addResponseAction(requestable, dataview);
        	...
        	ConvertigoPlugin.logException(e, "Unable to add action(s) to DreamFace!");
        }
        finally {
	...
	private String addResponseAction(RequestableObject requestable, String dataview) throws HttpException, IOException, ParserConfigurationException, SAXException, DreamFaceInvalidTemplateException {
		Hashtable<String, String> fields = getResponseFields(ADD_ACTION_TYPE, requestable, dataview);
