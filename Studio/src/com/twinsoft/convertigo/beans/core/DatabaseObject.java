...
public abstract class DatabaseObject implements Serializable, Cloneable {
	...
	public DatabaseObject() {
		try {
			...
			compilablePropertySourceValuesMap = new Hashtable<String, Object>(5);
    ...
    public Object clone() throws CloneNotSupportedException {
    	try {
    		...
    		((DatabaseObject)clone).compilablePropertySourceValuesMap = new Hashtable<String, Object>(5);
    ...
    public void delete() throws EngineException {
        ...
        if (!bResult) throw new EngineException("Unable to delete the object \"" + getQName() + "\". The file does not exist or is probably locked by another application.");
    }
    ...
    private void setBeanName(String name) throws EngineException {
    	if (name.length() == 0) throw new EngineException("The object name cannot be empty!");
        this.name = name;
    ...
    public void setName(String name) throws EngineException {
    	if (name == null) throw new EngineException("The object name cannot be null!");
		if (name.length() == 0) throw new EngineException("The object name cannot be empty!");
    ...
    public String getChildBeanName(Collection<? extends DatabaseObject> v,String dboName, boolean isNew) {
        ...
        int index = 0;
		while (bContinue) {
			if (index == 0) newDatabaseObjectName = dboName;
			else newDatabaseObjectName = dboName + index;
			try {
    ...
    public Element toXml(Document document) throws EngineException {
    	return toXml(document, false, false,false,false,false);
    }

    public Element toXml(Document document, boolean bIncludeDisplayName, boolean bIncludeCompiledValue, boolean bIncludeShortDescription,boolean bIncludeEditorClass,boolean bIncludeBlackListedElements) throws EngineException {
        ...
        for (int i = 0 ; i < len ; i++) {
            ...
            try {
            	...
            	if (isCipheredProperty(name) && !bIncludeDisplayName) {
                ...
                if (bIncludeBlackListedElements) {
        ...
        try {
            ...
            for (int i = 0 ; i < len ; i++) {
                ...
                if (childNodeName.equalsIgnoreCase("property")) {
                    ...
                    try {if ("false".equals(childAttributes.getNamedItem("traceable").getNodeValue())) maskValue = true;}
                    catch (Exception e) {}
    ...
    public static boolean valueIsCompilable(Object propertyObjectValue) {
    	if (propertyObjectValue instanceof String) {
			...
			for (int i=0; i<xmlv.size();i++) {
				...
				if (valueIsCompilable(ob)) return true;
			}
    	}
    	return false;
    }
			...
			try {
				...
				while ((i != -1) && (i < sPropertyObjectValue.length())) {
					...
					if (j == -1) {
						...
						if (symbolDefaultValue == null) symbolDefaultValue = "";
						if (symbolCompiledValue == null) symbolCompiledValue = symbolDefaultValue;
	...
	public static Object encryptPropertyValue(Object propertyValue) {
		...
		if (encryptedValue == null) encryptedValue = propertyValue;
	...
	public static Object decryptPropertyValue(Object encryptedValue) {
		...
		if (propertyValue == null) propertyValue = encryptedValue;
    ...
    protected static PropertyDescriptor findPropertyDescriptor(PropertyDescriptor[] pds, String pn) {
        ...
        for (int i = 0 ; i < len ; i++) {
            pd = pds[i];
            if (pn.equals(pd.getName())) return pd;
        }
        return null;
    }
	...
	protected <E extends Object> List<E> sort(List<E> list, boolean ascending){
		...
		if(!ascending) Collections.reverse(res);
		return res;
	}
