...
public class TransactionStep extends RequestableStep implements ITagsProperty{
					...
					if (httpState != null) {
						...
						for (int i=0; i<len; i++) {
							...
							if (cookie.getName().equalsIgnoreCase("JSESSIONID")) {
								Engine.logBeans.debug("Executing deletion of transaction's context of TranscationStep \""+ name +"\"");
								Engine.theApp.contextManager.removeAll(cookie.getValue());
								Engine.logBeans.debug("Deletion of transaction's context of TranscationStep \""+ name +"\" done");
								break;
							}
						}
					}
	...
	protected void prepareForRequestable(Context javascriptContext, Scriptable scope) throws MalformedURLException, EngineException {
		...
		Connector targetConnector = (connectorName.equals("") ? targetProject.getDefaultConnector():targetProject.getConnectorByName(connectorName));
		Transaction targetTransaction = (transactionName.equals("") ? targetConnector.getDefaultTransaction():targetConnector.getTransactionByName(transactionName));
		...
		if (bInternalInvoke ) {
	    	...
	    	request.put(Parameter.TransactionMotherSequenceContext.getName(), new String[] { sequence.context.contextID });
			...
			if (postQuery.indexOf(Parameter.TransactionMotherSequenceContext.getName()) == -1)
				postQuery = addParamToPostQuery(Parameter.TransactionMotherSequenceContext.getName(), sequence.context.contextID, postQuery);
	...
	public void importVariableDefinition() throws EngineException {
		...
		Transaction transaction = (transactionName.equals("") ? connector.getDefaultTransaction():connector.getTransactionByName(transactionName));
		...
		if (document == null) {
			...
			if (transaction.isPublicMethod()) {
				...
				try {
					...
					sx.replace("</xsd:schema>", "<xsd:element name=\"targetRequestable\" type=\"p_ns:"+ prefix + transaction.getName() +"Response\"/></xsd:schema>");
	...
	protected Node generateXmlFromXsd() throws EngineException {
		try {
			...
			Document xsdDom = xsd.generateTypeXmlStructure(projectName+"_ns", prefix + transaction.getName() +"ResponseData");
	...
	protected byte[] executeMethod() throws IOException, URIException, MalformedURLException, EngineException {
		...
		if (sequence.runningThread.bContinue) {
			Engine.logBeans.debug("(TransactionStep) Executing method - "+ method.getName() +"("+ method.getPath()+")");
			...
			try {
				...
				if (statuscode >= 300) {
					throw new EngineException("(TransactionStep) HTTP response returned status :"+ ((method != null) ? method.getStatusLine().toString():String.valueOf(statuscode)));
				}
			}
			finally {
				if (method != null)
	...
	public String[] getTagsForProperty(String propertyName) {
		...
		}else if(propertyName.equals("transactionName")){
			return getTransactionNames();
		}
	...
	public void addSchemaType(HashMap<Long, String> stepTypes, String tns, String occurs) throws EngineException {
		...
		stepTypeSchema += "\t\t\t<xsd:element name=\"document\" minOccurs=\"0\" type=\""+ p.getName() + "_ns:" + prefix + transaction.getName() +"ResponseData\">\n";
		stepTypeSchema += "\t\t\t\t<xsd:annotation>\n";
		stepTypeSchema += "\t\t\t\t\t<xsd:documentation>"+ XMLUtils.getCDataXml(transaction.getComment()) +"</xsd:documentation>\n";
		stepTypeSchema += "\t\t\t\t</xsd:annotation>\n";
