...
public class JavelinConnectorComposite extends AbstractConnectorComposite implements zoneListener, twinxListener, KeyListener, IConnectable, IRecordable, IRefreshable, IScreenClassAware, IBlockizable, ILinkable, IAddable {

	public JavelinConnectorComposite(ConnectorEditorPart connectorEditorPart, Connector connector, Composite parent, int style) {
	...
	protected void initJavelin(boolean bRecord) {
		...
		if (bRecord) {
			...
			traceFilePath = Engine.PROJECTS_PATH + "/" + connector.getProject().getName() + "/Traces/" + connector.getName() + "/";
		...
		// The following code allows to push relevant MDC values into Javelin's threads
		JavelinObjectCreatorThread javelinObjectCreatorThread = new JavelinObjectCreatorThread(
				this, traceFilePath);
	...
	class JavelinObjectCreatorThread extends Thread {
		...
		public JavelinObjectCreatorThread(JavelinConnectorComposite javelinConnectorComposite, String traceFilePath) {
		...
		public void run() {
			...
			if (javelin == null) {
				try {
					...
					if (isPlaying) {
			            String tracePlayerPort = ConvertigoPlugin.getProperty(ConvertigoPlugin.PREFERENCE_TRACEPLAYER_PORT);
					...
					Authentication auth = Engine.theApp.getAuthenticationObject(
						javelinConnector.getVirtualServer(),
						null
					);
					...
					sessionId = ConnectorEditorPart.CONVERTIGO_STUDIO_SESSION_ID + "_" + connector.getParent().getName() + ":" + connector.getName();
					...
					if (session != null) {
						...
						set.add(AWTKeyStroke.getAWTKeyStroke(KeyEvent.VK_TAB, KeyEvent.CTRL_MASK | KeyEvent.SHIFT_MASK, false));
						...
						String message = "Unable to add a new session: the service '" + serviceCode + "' is probably not authorized.";
						javelin = null;
    ...
    public void clear() {
        ...
        if (javelin != null) {
            ...
            try {
                ...
                // Silently ignore: if the Studio is being closed, the Engine could
                // be yet closed when this method is called, that's why the Engine
...
//            	redraw();
	...
	class JavelinObjectConnectThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				else javelinConnector.javelin.connect(timeoutForConnect);
				
				if (!javelinConnector.javelin.isConnected()) {
					throw new EngineException("Unable to connect the connector! See the connector console for more details...");
				}
				javelinConnector.javelin.requestFocus();
			}
			catch(Throwable e) {
				ConvertigoPlugin.logException(e, "An unexpected error has occured during the connection of the emulator.");
			}
			finally {
				Display.getDefault().asyncExec(new Runnable(){
					public void run() {
	...
	class JavelinObjectDisconnectThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				javelinConnector.javelin.disconnect();
				if (javelinConnector.javelin.isConnected()) {
					throw new EngineException("Unable to disconnect the connector! See the connector console for more details...");
				}
				...
				ConvertigoPlugin.logException(e, "An unexpected error has occured during the disconnection of the emulator.");
			}
			finally {
				Display.getDefault().asyncExec(new Runnable(){
					public void run() {
	...
	/**
	 * Sets the value of the selected property with the selected zone from the connector
	 */
	public void link() {
		try {
			if (connector != null) {
				...
				if (pev != null) {
					// gets the databaseObject of the selected element in project tree 
					...
					// gets the propertyDescriptor of the selected property in the Properties panel
		    		...
		    		if (propertyDescriptor != null) {
	    				...
	    				// test if the selected property is linkable with a screen selection zone
	    				if (editorClass != null) {
	    					...
	    					if (propertySelectionModifierGetValueMethod != null) {
	    						...
	    						// the selected property is not linkable with a screen selection zone
	    					}
		    			...
		    			// no PropertyDescriptor corresponding to the selected property
		    		}
				}
				else {
					// pev null
	...
	/**
	 * Adds an element to the selected property with the selected zone from the connector
	 */
	public void add() {
		try {
			if (connector != null) {
				...
				if (pev != null) {
		    		...
		    		if (propertyDescriptor != null) {
		    			// test if the selected property is addable with a screen selection zone (we can add an element to the property from the selected zone) 
	    				Class<?> editorClass = propertyDescriptor.getPropertyEditorClass();
	    				if (editorClass != null) {
	    					...
	    					ConvertigoPlugin.getDefault().getPropertiesView().selectionChanged((IWorkbenchPart)pev, structuredSelection);
...
//		 TODO Auto-generated method stub
    ...
    private static final ActionTbl[][] KEYS_TABLE = {
        null, // emulatorID = 0
        new ActionTbl[] { // emulatorID = 1, VDX
            ...
            new ActionTbl("KAnnulation", KeyEvent.VK_RIGHT, false,false,false,  0,false,false,false),
            new ActionTbl("KRepetition", KeyEvent.VK_TAB, false,false,false,  0,false,false,false),
            new ActionTbl("KCorrection", KeyEvent.VK_BACK_SPACE, false,false,false,  0,false,false,false),
            new ActionTbl("KCnxFin", KeyEvent.VK_F9, false,false,false,  0,false,false,false),
            null
        },
        new ActionTbl[] { // emulatorID = 2, VT
        ...
        new ActionTbl[] { // emulatorID = 3, Bull
            // Function Keys
            ...
            new ActionTbl("CLEAREP", KeyEvent.VK_PAGE_DOWN		,false,false,false,  0,false,false,false),
            ...
            new ActionTbl("FILEXFER_RECEIVE", KeyEvent.VK_F12			,false,true,false,  0,false,false,false),
            null
        },
        null,  // emulatorID = 4, HTML
        new ActionTbl[] { // emulatorID = 5, 3270
            ...
            new ActionTbl("KEY_CURRIGHT", KeyEvent.VK_RIGHT,					false,false,false,  0,false,false,false),
            ...
            new ActionTbl("KEY_BACKSP", KeyEvent.VK_BACK_SPACE,					false,false,false,  0,false,false,false),
            ...
            new ActionTbl("KEY_INSERT", KeyEvent.VK_INSERT,						false,false,false,  0,false,false,false),
            new ActionTbl("KEY_DELCHAR", KeyEvent.VK_DELETE,					false,false,false,  0,false,false,false),
            ...
            new ActionTbl("KEY_ERASEEOF", KeyEvent.VK_DELETE,					true ,false,false,  0,false,false,false),
            new ActionTbl("KEY_ERASEINPUT", KeyEvent.VK_END,					false,false,true ,  0,false,false,false),
            ...
            new ActionTbl("KEY_ROLLUP", KeyEvent.VK_PAGE_UP,					false,false,false,  0,false,false,false),
            new ActionTbl("KEY_ROLLDOWN", KeyEvent.VK_PAGE_DOWN,				false,false,false,  0,false,false,false),
            null
        },
        null,  // emulatorID = 6, 3287
                ...
                if (Character.isLetterOrDigit(c) || c == ' ') {
                    ...
                    if (doActionName != null) {
                    	if (learnScreenClass == null) {
                    		...
                    		//learnScreenClass = projectExplorerView.getLastDetectedScreenClass();
                    		learnScreenClass = currentScreenClass;
                    		openScreenClassHandler(true);
                    	}
                    	
                        // Writing wait sync function if needed (only for videotex)
    ...
    private void openScreenClassHandler(boolean bEntry) {
        ...
        if (bEntry) {
            String line = java.text.MessageFormat.format(
                "// Entry handler for screen class \"{0}\"",
            ...
            String line = java.text.MessageFormat.format(
                "// Exit handler for the screen class \"{0}\"",
                new Object[] { learnScreenClassName }
            );
            writeLine("\n" + line);
    ...
    private void closeScreenClassHandler(String returnedVerb) {
        ...
        if (returnedVerb == null) writeLine("return;");
        else writeLine("return \"" + returnedVerb + "\";");
        writeLine("}");
    ...
    public void createWaitAtFromSelectionZone() {
    	...
    	if (javelinConnector.isLearning()) {
            ...
            if (!bHandlerOpened) openScreenClassHandler(true);
...
//				if (projectExplorerView != null) {
//					BeanInfo bi = projectExplorerView.getFirstSelectedDatabaseObjectBeanInfo();
//					if (bi != null) {
				...
				if (projectExplorerView != null) {
					...
					if (bi != null) {
				        ...
				        try {
				        	enable = pd != null &&
						        	bSelected && 
//				            		getLastDetectedScreenClass() != null && // seems to be broken and not very usefull, commented for bug #952 
				            		!javelin.getSelectionZone().isEmpty() &&
				            		(
				            			pd.getPropertyEditorClass().isAssignableFrom(
				            					Class.forName("com.twinsoft.convertigo.eclipse.property_editors.ColumnEditor")
				            			)
				            		);
				        } catch (ClassNotFoundException e) {
				        	enable = false;
	...
	/* (non-Javadoc)
	 * @see com.twinsoft.convertigo.eclipse.editors.connector.AbstractConnectorComposite#setAccumulate(boolean)
	 */
	...
	/* (non-Javadoc)
	 * @see com.twinsoft.convertigo.eclipse.editors.connector.AbstractConnectorComposite#startLearn()
	 */
	...
	/* (non-Javadoc)
	 * @see com.twinsoft.convertigo.eclipse.editors.connector.AbstractConnectorComposite#stopLearn()
	 */
	public void stopLearn() {
		if (!handlers.equals("")) {
			// Is there still a screen class handler being generated?
			if (bHandlerOpened) closeScreenClassHandler();
