<script>
	...
	const compactHeight = 480;
	const minViewportHeight = 240;

	let fitViewport = $state(false);
	let viewportHeight = $state(compactHeight);
	let shell = $state();
	let editorHeight = $derived(
		fitViewport
			? Math.max(minViewportHeight, Math.floor(viewportHeight || compactHeight))
			: compactHeight
	);
...
<MaxHeight bind:height={viewportHeight}>
	<div
