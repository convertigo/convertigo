					...
					if (isOutsideContainer(parentDbo.classname)) {
						...
						if (lastChildDbo !== undefined) {
							if (isOutsideContainer(lastChildDbo.classname)) {
								edges.push(
									createEdge(lastChildDbo.id, childDbo.id, lastChildDbo?.children?.length > 0)
								);
...
export function getLayoutedElements(nodes, edges, options = {}) {
	...
	const flatten = (items) => {
		...
		const flattened = flattenedItems.concat(children.length ? flatten(children) : children);
		return flattened;
	};
	...
	return elk
		...
		.then((layoutedGraph) => {
			...
			const svelteNodes = flattenedElkNodes.map((n) => ({
				...
				//position: { x: n.x + (n.parentId ? 0:0), y: n.y },
				//style: "width: 150px; height: 40px",
