	...
	<div class="flex flex-col grid md:grid-cols-5 gap-5">
		<div class="flex flex-col h-auto md:col-span-4">
			<Card title={category['@_displayName']}>
				<div class="flex flex-row justify-around flex-wrap mb-5">
					<button
						type="button"
						disabled={!hasChanges}
						class="btn p-1 pl-5 pr-5 w-80 bg-buttons font-normal rounded-full font-medium"
					...
					<button
						type="button"
						disabled={!hasChanges}
						class="btn p-1 pl-5 pr-5 w-80 variant-filled-error font-normal rounded-full font-medium"
