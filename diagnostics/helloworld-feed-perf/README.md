...
### Global block artifact preload

The AutoStart warmups were removed after replacing them with a runtime-level
cache. Compiled block artifacts are now shared by provider QName and source
fingerprint; the fingerprint also includes the block compiler implementation,
so changing either source or compiler invalidates the artifact. The
`FlowEngine` owned by the provider project preloads its catalog after the full
DBO tree is loaded. Consumer projects do not run another preload and reuse the
same immutable artifacts.

The staged benchmark restarted the local runtime before each step and used the
same 46 KB RSS fixture over the Docker bridge. It therefore excludes external
NASA latency. Every response was 7,218 bytes with the same SHA-256 digest.

| Step | Change | First GetFeed | Warm GetFeed | Change vs P0 |
| --- | --- | ---: | ---: | ---: |
| P0 | No AutoStart, no global artifact cache | 5,062 ms | 31-39 ms | baseline |
| P1 | Global block artifacts only | 4,559 ms | 34-41 ms | -9.9% |
| P2 | Rejected: every consumer preloads the shared runtime | 9,500 ms | 35-40 ms | +87.7% |
| P3 | Provider-owned preload only | 1,229-1,285 ms | 23-37 ms | -74.6% to -75.7% |

P2 exposed five concurrent project preload calls serialized by the shared
Rhino runtime lock. Restricting the hook to the project named by the engine
QName removed that contention. The single `lib_flow_engine` preload took
3,432-3,883 ms during server project loading and prepared 103 core blocks. The
remaining 1.2-1.3 seconds on the first application request is project-specific
loading and compilation, not external HTTP or repeated parsing of core blocks.

Artifact: `results/flow-global-block-cache-20260714.json`.

### First-request phase profile and QName plan cache

The remaining cold time was profiled with `__flowProfile=true`. This mode is
opt-in and does not add profile data to normal Flow responses. The controlled
fixture remains local HTTP over the Docker bridge, so none of these values
contains DNS or TLS setup against NASA.

| Step | Change | First GetFeed | `compilePlan` | `executeNodes` |
| --- | --- | ---: | ---: | ---: |
| P6 | Skip the input bridge for a statically declared no-input Flow | 836-911 ms | - | - |
| P7 | Internal phase profile, no additional optimization | 782 ms | 309 ms | 131 ms |
| P9 | Prepare the generic compiler during provider preload | 616 ms | 200 ms | 128 ms |
| P10 | Compile and cache each Flow plan by QName after provider preload | 378-400 ms | 0.4-0.9 ms | 110-158 ms |

P7 accounts for the suspected 600 ms: 309 ms compiled the first FlowScript
plan, 131 ms executed its nodes (99 ms local HTTP and 23 ms XML projection),
82 ms loaded the project catalog, and 18 ms loaded config and created the run
context. The P10 plan cache removes that compilation from the request without
executing a Flow or performing external I/O. Plans are prepared only after the
provider has loaded the immutable core block artifacts, and preparation is
serialized to avoid the global Rhino-lock contention observed in P2.

On this development workspace, migration then spends 4.0 seconds loading the
103-block provider and another 4.0 seconds preparing nine registered Flows.
Most plans take 14-155 ms; `lib_flow_mcp.McpServer` contributes 2.8 seconds
because its project catalog contains 101 additional blocks. This is deliberate
startup work rather than hidden first-request latency, but it remains a startup
cost to optimize for large block libraries.

The P10 response is still 7,218 bytes with SHA-256
`c2dd0fd76f9b1ab622c372d0b7639404a36baa41b9a4228edb4cc8d2d07eae57`.
The next four calls take 20-28 ms. A Flow with declared inputs
(`lib_flow_mcp.McpServer`) was also invoked after the Java metadata fast path;
its JSON-RPC id was preserved, confirming that non-empty inputs still use the
full synchronization path.

Artifacts: `results/saved-local-internal-profile-p7-20260714T115100Z.*`,
`results/saved-local-compiler-preload-p9-20260714T115600Z.*`, and
`results/saved-local-qname-validated-p10c-20260714T120300Z.*`.

### Java envelope and lazy project block catalogs

P11 added an opt-in Java profile around the Flow bridge and JSON-to-XML
conversion. On the first controlled GetFeed, `runCore` took 136.5 ms, including
135.4 ms in the bridge and only 1.1 ms in JSON-to-XML. The bridge itself spent
130.4 ms in `runFlowRequest`; constructing the Java request and copying inputs
took less than 0.5 ms. The remaining first-request wall time is therefore in
the generic Convertigo request/context/session envelope, not in a hidden Flow
Java or JSON/XML phase. Warm JSON-to-XML conversion stayed between 0.5 and
1.5 ms.

P12 changes project and referenced block catalogs from eagerly compiled maps
to descriptor proxies. Reserving a block now indexes its QName and source file
without reading or compiling the source. Metadata is extracted only when the
parser, picker or catalog asks for that block. Preparing a Flow compiles its
direct Rhino blocks, while Flow composite blocks and dynamically selected
blocks compile on first use. All compiled artifacts remain global and keyed by
provider QName plus source/compiler fingerprint.

| Stage | `lib_flow_mcp.McpServer` prepare | Nine registered plans | First local GetFeed | Warm local GetFeed |
| --- | ---: | ---: | ---: | ---: |
| P10 eager project catalog | 2,743-2,820 ms | about 4,006 ms | 378-400 ms | 20-28 ms |
| P12 lazy descriptors and targeted Rhino compile | 217-300 ms | 989-1,027 ms | 414 ms | 23-33 ms |

The large MCP plan preparation drops by 89.4-92.3%, and registered-plan work
drops by roughly 75%, without changing GetFeed output. The P12 GetFeed response
is still 7,218 bytes with SHA-256
`c2dd0fd76f9b1ab622c372d0b7639404a36baa41b9a4228edb4cc8d2d07eae57`.
Its first-run Flow profile was 162 ms, including 100 ms in the first local HTTP
block, 19 ms in the NASA XML projection and 1.7 ms in JSON-to-XML; the 414 ms
wall value is within the cold generic-envelope variance already observed.

The tradeoff is explicit: the first MCP `initialize` branch took 1,183 ms and
the first `tools/list` branch 703 ms because their Flow composites were compiled
when first selected. Subsequent dynamic dispatch worked normally; a
`tools/call` to `flow-search` returned one result in 408 ms on its first use.
This avoids parsing 101 unrelated MCP blocks during every startup. A future
background readiness policy may choose to compile selected library entrypoints,
but should not restore exhaustive project-catalog compilation.

Artifacts: `results/saved-local-java-envelope-p11-20260714T120700Z.*` and
`results/flow-lazy-project-blocks-20260714.json`.

The earlier AutoStart version of the engine and Hello World project was
deployed to beta after backing up
...
## Historical Flow runtime AutoStart warmup

This experiment is superseded by the provider-owned global artifact preload
above; the AutoStart sequences have been removed.
