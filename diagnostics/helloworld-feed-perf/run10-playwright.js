...
const project = "sample_HelloWorldFlowRun10";
const appUrl =
  ...
  return page.evaluate((payload) => {
    ...
    return {
      ...
      alternates: rows.every((row, index) =>
        row.variant === (index % 2 === 0 ? "sky" : "dark")),
...
async function main() {
  ...
  try {
    ...
    await page.screenshot({
      path: "diagnostics/helloworld-feed-perf/results/run10-desktop.png",
      fullPage: false
    });
    await page.setViewportSize({ width: 390, height: 844 });
    const mobile = await inspect(page, payload);
    await page.screenshot({
      path: "diagnostics/helloworld-feed-perf/results/run10-mobile.png",
      fullPage: false
    });
    ...
    for (const result of [desktop, mobile]) {
      ...
      assert.deepEqual(result.variantCounts, { sky: 30, dark: 30 });
