...
const appUrl =
  ...
  "sample_HelloWorldFlowRun7/DisplayObjects/mobile/index.html";
...
async function inspect(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      realTitleCount: Array.from(document.querySelectorAll("body *")).filter(
        (node) => node.children.length === 0 && node.textContent === "NASA Astronaut Anil Menon"
      ).length,
      placeholderTitleCount: Array.from(document.querySelectorAll("body *")).filter(
        (node) => node.children.length === 0 && node.textContent === "NASA story"
      ).length,
...
async function main() {
  ...
  page.on("response", (response) => {
    if (response.url().includes("sample_HelloWorldFlowRun7/.json")) {
  ...
  try {
    ...
    const responsePromise = page.waitForResponse(
      (response) => response.url().includes("sample_HelloWorldFlowRun7/.json") && response.status() === 200,
      { timeout: 30_000 }
    );
    await page.getByRole("button", { name: "Load NASA News" }).click();
    await responsePromise;
    await page.getByText("NASA Astronaut Anil Menon", { exact: true }).first().waitFor({ timeout: 30_000 });
    await page.locator("img").first().waitFor({ timeout: 30_000 });
    await page.waitForTimeout(2_000);

    const desktop = await inspect(page);
    await page.screenshot({
      path: "diagnostics/helloworld-feed-perf/results/run7-desktop.png",
      fullPage: false
    });
    ...
    await page.screenshot({
      path: "diagnostics/helloworld-feed-perf/results/run7-mobile.png",
      fullPage: false
    });
    ...
    const report = {
      ...
      functionalContent: {
        ...
        realTitleCount: desktop.realTitleCount,
    ...
    assert.equal(desktop.placeholderTitleCount, 0);
    assert.equal(desktop.realTitleCount > 0, true);
