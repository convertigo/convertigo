...
const url = process.env.RETAILSTORE_URL || "http://127.0.0.1:19080/convertigo/projects/sample_RetailStoreFlowRun9/DisplayObjects/mobile/";
const profile = path.join(os.tmpdir(), "retailstore-flow-run9-independent-profile");
...
(async () => {
  ...
  await page.screenshot({ path: path.join(evidence, "run9-root-mobile.png"), fullPage: true });
  ...
  const productImages = page.locator('img[alt="Catalog item"]:visible');
  ...
  await page.screenshot({ path: path.join(evidence, "run9-products-mobile.png"), fullPage: true });
  ...
  await page.screenshot({ path: path.join(evidence, "run9-detail-mobile.png"), fullPage: true });
  ...
  await page.getByRole("button", { name: product, exact: true }).click();
  await page.waitForURL(/\/detail\/?$/);
  const visibleBack = page.getByText("Back", { exact: true });
  ...
  await clickBranch(page);
  check(await page.locator('img[alt="Catalog item"]:visible').count() === 22, "offline-local-views");
  await page.getByRole("button", { name: product, exact: true }).click();
  await page.waitForURL(/\/detail\/?$/);
  ...
  await page.screenshot({ path: path.join(evidence, "run9-detail-desktop.png"), fullPage: true });
  await context.close();
  ...
  console.log(`RUN9_INDEPENDENT_STATE=${JSON.stringify(result)}`);
