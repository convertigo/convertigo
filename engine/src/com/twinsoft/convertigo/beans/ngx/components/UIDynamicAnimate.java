...
public class UIDynamicAnimate extends UIDynamicAction {
	...
	protected StringBuilder initProps(boolean forTemplate) {
		...
		if (forTemplate) {
			...
			sbProps.append(isEmpty ? "null": "scope."+ animatableId + " ? scope."+ animatableId + (!this.isTplStandalone() ? " : get('animatable',`c8oPage."+ animatableId+"`)" : " : c8oPage."+ animatableId+")"));
		}
		...
		if (forTemplate) {
			...
			sbProps.append(isEmpty ? "null": (!this.isTplStandalone() ? "get('animatables', `c8oPage.all_"+animatableId+"`)" :"c8oPage.all_"+animatableId+")"));
		}
